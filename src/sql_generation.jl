...
function build_sql(nodes::Vector{QueryableBackend.Queryable}, params::Vector{Any})
    ...
    from_clause = source_to_from(source)
    ...
    for i in 2:length(nodes)
        ...
        elseif node isa QueryableBackend.QueryableMap
            if select_clause != "*"
                ...
                inner_sql = assemble_sql(select_clause, from_clause, where_clauses, groupby_clauses, orderby_clauses, limit_clause, offset_clause, distinct)
            ...
            select_clause = translate_map_expr(node.f_expr, params)
        ...
        elseif node isa QueryableBackend.QueryableJoin
            ...
            inner_from = if inner_source isa DuckDBQueryableSource
                source_to_from(inner_source)
            else
                throw(TranslationError("JOIN inner source must be a DuckDBQueryableSource", :join))
            end
            # Extract key selectors
            outer_key = translate_orderby_expr(node.outerKeySelector_expr, params)
            inner_key = translate_orderby_expr(node.innerKeySelector_expr, params)
            from_clause = "$from_clause INNER JOIN $inner_from ON $outer_key = $inner_key"
    ...
    sql = assemble_sql(select_clause, from_clause, where_clauses, groupby_clauses, orderby_clauses, limit_clause, offset_clause, distinct)
...
function source_to_from(source::DuckDBQueryableSource)
...
function assemble_sql(select_clause, from_clause, where_clauses, groupby_clauses, orderby_clauses, limit_clause, offset_clause, distinct)
    parts = String[]
